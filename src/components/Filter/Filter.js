...
export const Filter = (props) => {
  ...
  const { walletAddress, synchroTables, handleSynchroTables } =
    ...
    if (categorySelected) {
      switch (filterOption) {
        case 6:
          ...
          dbRef
            .orderByChild('listed')
            .startAt(Math.floor(epochNow / 86400) * 86400)
            .on('value', (snapshot) => {
              if (snapshot.exists()) {
                setAllData(snapshot.val());
  ...
  const setAllShowDataFunc = async (data) => {
    ...
    for (let i in data) {
      if (diamondSelected) {
        ...
        if (
          data[i].voteCount < ENVS.RUBY_TIRE_LIMIT ||
          data[i].voteCount >= ENVS.DIAMOND_TIRE_LIMIT
        )
          continue;
  ...
  const handleOpenWatchListDlg = (id) => {
    ...
    if (wllist.includes(walletAddress)) {
      setWlDlgTitle(
        'You are going to remove this project from your Watchlist?'
      );
    } else {
  ...
  const voteClicked = async (id) => {
    ...
    await signer
      ...
      .then(async () => {
        ...
        if (
          Number(allData[id].voteCount) < ENVS.DIAMOND_TIRE_LIMIT &&
          Number(allData[id].voteCount) + 1 >= ENVS.DIAMOND_TIRE_LIMIT
        ) {
          await fetch(
            `${process.env.REACT_APP_SERVER_URL}/diamond/?token=${allData[id].name}`,
            {
              method: 'GET',
            }
          )
            .then(() => {
              NotificationManager.info(
                'Notification about Diamond Tier has sent to TG channel.'
              );
            })
            .catch((err) => console.log(err));
        } else if (
          Number(allData[id].voteCount) < ENVS.RUBY_TIRE_LIMIT &&
          Number(allData[id].voteCount) + 1 >= ENVS.RUBY_TIRE_LIMIT
        ) {
          await fetch(
            `${process.env.REACT_APP_SERVER_URL}/ruby/?token=${allData[id].name}`,
            {
              method: 'GET',
            }
          )
            .then(() => {
              NotificationManager.info(
                'Notification about Ruby has sent to TG channel.'
              );
            })
            .catch((err) => console.log(err));
        } else {
  ...
  const columns = [
    ...
    'MCAP',
    {
      ...
      options: {
        customBodyRender: (value, tableMeta, updateValue) => {
          ...
          if (isNaN(val)) {
            return (
              <div className="change24hDiv" style={{ color: 'white' }}>
                <span>{val}</span>
              </div>
            );
          } else if (val < 0) {
            val = 0 - val;
            return (
              <div className="change24hDiv" style={{ color: 'yellow' }}>
                <span>{'- ' + val + ' %'}</span>
              </div>
            );
          } else {
            return (
              <div className="change24hDiv" style={{ color: 'green' }}>
    ...
    'Launch',
    {
      ...
      options: {
        customBodyRender: (value, tableMeta, updateValue) => {
          return (
            <div className="tierImgDiv">
              <img src={tableMeta.rowData[6]} alt="" />
            </div>
          );
        },
      },
        ...
        customBodyRender: (value, tableMeta, updateValue) => {
          return (
            <div className="wlImgDiv">
              ...
              <button
                style={{
                  backgroundImage: `url(${tableMeta.rowData[9]})`,
                  width: '40px',
                  height: '40px',
          ...
          <div className="filterOptionDiv">
            <div className="diamondFilterDiv">
              {diamondSelected === true ? (
                <div
                  ...
                  style={{
                    backgroundImage:
                      'linear-gradient(to right, #DC1Bd9, #4EB4C3)',
                  }}
            ...
            >
              ...
              <div
                ...
                style={{ display: showNetworkSubFilter }}
              >
            ...
            >
              ...
              <div
                ...
                style={{ display: showCategorySubFilter }}
              >
